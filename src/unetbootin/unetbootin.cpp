...
// Helper function to add category separator to dropdown
void unetbootin::addCategorySeparator(const QString& categoryName) {
	// Add the separator item
	distroselect->addItem(categoryName, QStringList() << "");
	
	// Get the model and last item
	QStandardItemModel *model = qobject_cast<QStandardItemModel*>(distroselect->model());
	if (model) {
		...
		if (item) {
			...
			// Style it - bold and gray background
			...
			// Dark gray text
			item->setForeground(QBrush(QColor(80, 80, 80)));
		}
	}
}
