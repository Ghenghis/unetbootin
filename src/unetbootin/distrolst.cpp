...
if (nameDistro == "CentOS Stream")
{
	...
	QString streamVersion = relname.section('_', 0, 0);
	
	if (islivecd)
	{
		downloadfile(fileFilterNetDir(QStringList() <<
		QString("https://mirrors.centos.org/mirrorlist?path=/%1-stream/isos/%2/&redirect=1").arg(streamVersion, cpuarch) <<
		QString("https://mirror.stream.centos.org/%1-stream/BaseOS/%2/iso/").arg(streamVersion, cpuarch) <<
		QString("https://mirrors.kernel.org/centos-stream/%1-stream/BaseOS/%2/iso/").arg(streamVersion, cpuarch)
		, 524288000, 2684354560, QList<QRegExp>() <<
		QRegExp(".iso$", Qt::CaseInsensitive) <<
		QRegExp("CentOS-Stream.*boot\\.iso$", Qt::CaseInsensitive) <<
		QRegExp("CentOS-Stream.*dvd.*\\.iso$", Qt::CaseInsensitive)
		), isotmpf);
		extractiso(isotmpf);
	}
	else
	{
		downloadfile(QString("https://mirror.stream.centos.org/%1-stream/BaseOS/%2/os/images/pxeboot/vmlinuz").arg(streamVersion, cpuarch), QString("%1ubnkern").arg(targetPath));
		downloadfile(QString("https://mirror.stream.centos.org/%1-stream/BaseOS/%2/os/images/pxeboot/initrd.img").arg(streamVersion, cpuarch), QString("%1ubninit").arg(targetPath));
		postinstmsg = unetbootin::tr("\n*IMPORTANT* After rebooting, select 'Install CentOS Stream %1' from the boot menu. Network installation will proceed from official CentOS mirrors. Ensure you have an active internet connection during installation.").arg(streamVersion);
...
if (nameDistro == "Rocky Linux")
{
	...
	QString rockyVersion = relname.section('_', 0, 0);
	
	if (islivecd)
	{
		downloadfile(fileFilterNetDir(QStringList() <<
		QString("https://download.rockylinux.org/pub/rocky/%1/isos/%2/").arg(rockyVersion, cpuarch) <<
		QString("https://dl.rockylinux.org/pub/rocky/%1/isos/%2/").arg(rockyVersion, cpuarch)
		, 524288000, 2684354560, QList<QRegExp>() <<
		QRegExp(".iso$", Qt::CaseInsensitive) <<
		QRegExp("Rocky.*boot\\.iso$", Qt::CaseInsensitive) <<
		QRegExp("Rocky.*dvd.*\\.iso$", Qt::CaseInsensitive) <<
		QRegExp("Rocky.*minimal.*\\.iso$", Qt::CaseInsensitive)
		), isotmpf);
		extractiso(isotmpf);
	}
	else
	{
		downloadfile(QString("https://download.rockylinux.org/pub/rocky/%1/BaseOS/%2/os/images/pxeboot/vmlinuz").arg(rockyVersion, cpuarch), QString("%1ubnkern").arg(targetPath));
		downloadfile(QString("https://download.rockylinux.org/pub/rocky/%1/BaseOS/%2/os/images/pxeboot/initrd.img").arg(rockyVersion, cpuarch), QString("%1ubninit").arg(targetPath));
		postinstmsg = unetbootin::tr("\n*IMPORTANT* After rebooting, select 'Install Rocky Linux %1' from the boot menu. The installation wizard will guide you through the setup process. For network installation, ensure you have an active internet connection.").arg(rockyVersion);
		kernelOpts = "splash=silent showopts";
	}
}
...
if (nameDistro == "AlmaLinux")
{
	...
	QString almaVersion = relname.section('_', 0, 0);
	
	if (islivecd)
	{
		downloadfile(fileFilterNetDir(QStringList() <<
		QString("https://repo.almalinux.org/almalinux/%1/isos/%2/").arg(almaVersion, cpuarch) <<
		QString("https://mirrors.almalinux.org/isos/%1/%2/").arg(almaVersion, cpuarch)
		, 524288000, 2684354560, QList<QRegExp>() <<
		QRegExp(".iso$", Qt::CaseInsensitive) <<
		QRegExp("AlmaLinux.*boot\\.iso$", Qt::CaseInsensitive) <<
		QRegExp("AlmaLinux.*dvd.*\\.iso$", Qt::CaseInsensitive) <<
		QRegExp("AlmaLinux.*minimal.*\\.iso$", Qt::CaseInsensitive)
		), isotmpf);
		extractiso(isotmpf);
	}
	else
	{
		downloadfile(QString("https://repo.almalinux.org/almalinux/%1/BaseOS/%2/os/images/pxeboot/vmlinuz").arg(almaVersion, cpuarch), QString("%1ubnkern").arg(targetPath));
		downloadfile(QString("https://repo.almalinux.org/almalinux/%1/BaseOS/%2/os/images/pxeboot/initrd.img").arg(almaVersion, cpuarch), QString("%1ubninit").arg(targetPath));
		postinstmsg = unetbootin::tr("\n*IMPORTANT* After rebooting, select 'Install AlmaLinux %1' from the boot menu. Network installation will proceed automatically from AlmaLinux official mirrors. Ensure you have an active internet connection during the installation process.").arg(almaVersion);
		kernelOpts = "splash=silent showopts";
	}
}
